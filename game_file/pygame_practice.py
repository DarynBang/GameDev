...
class Game:
    def __init__(self):
        ...
        self.assets = {
            ...
            'particles/leaf': Animation(load_images('particles/leaf'), img_duration=24, loop=False)
        }
        ...
        while True:
            ...
            for rect in self.leaf_spawners:
                if random.random() * 29999 < rect.width * rect.height:
                    ...
                    leaf_velocity = [random.randint(-5, -1) / 10, random.randint(3, 6) / 10]
                    self.particles.append(Particle(self, 'leaf', spawn_pos, leaf_velocity, frame=random.randint(0, 20)))
            ...
            for particle in self.particles.copy():
                # Check if we have to kill a particle
                kill = particle.update()
                particle.render(self.display, offset=rendered_scroll)
